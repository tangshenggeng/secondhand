                ...
                <div class="row">
					<div class="col-lg-12">
						<form class="layui-form" id="kwCashForm">
                            ...
                            <div class="layui-inline layui-show-xs-block">
                                <input type="text" name="name" placeholder="输入关键字" autocomplete="off" class="layui-input">
                            </div>
                            <div class="layui-inline layui-show-xs-block">
		...
		layui.use(['table','util'], function() {
			...
			table.on('tool(rechargeListTbFilter)', function(obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
				...
				if (layEvent === 'detail') { //查看
					var apprId = data.apprId
					$.ajax({
						url:"${PATH}/appraisal/getById/"+apprId,
						method:"get",
						success:function(res){
							if(res.code==100){
								$("#custNowDiv").html(res.extend.appr.apprIntegral);
								$("#custNickDiv").html(res.extend.appr.apprNick);
								$("#custNameDiv").html(res.extend.appr.apprName);
							}
						},error:function(){
					...
					$.ajax({
						url:"${PATH}/cashAppr/exceptionCash",
						method:"post",
						contentType: "application/json",//必须指定，否则会报415错误
				        dataType : 'json',
	...
	$("#delByIds").click(function(){
		layui.use(['table','layer'], function() {
			...
			layer.confirm('真的删除行么', function(index){
				...
				$.ajax({
					url:"${PATH}/cashAppr/delCashByIds",
					method:"POST",
					contentType: "application/json",//必须指定，否则会报415错误
				    dataType : 'json',
...
<button type="button" class="btn btn-success btn-xs btn-outline" lay-event="success">成功</button>
