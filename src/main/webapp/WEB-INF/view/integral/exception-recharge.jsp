	...
	function renderTb() {
		...
		layui.use(['table','util'], function() {
			...
			table.render({
				elem : '#rechargeListTb',
				height : 600,
				url : '${PATH}/rechargeCust/getExceptionRechargeList',
				text : {
					none : '未找到充值信息'
				},
				id:"rechargeListTbId",
			...
			table.on('tool(rechargeListTbFilter)', function(obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
				...
				if (layEvent === 'detail') { //查看
					...
					$.ajax({
						url:"${PATH}/rechargeCust/recoveryRecharge",
						method:"post",
						contentType: "application/json",//必须指定，否则会报415错误
				        dataType : 'json',
...
<button type="button" class="btn btn-warning btn-xs btn-outline" lay-event="reject">恢复</button>
</div>
</script>
</html>
